...
/** Clases de botón, también para enlaces que se ven como botón (<Link className={clasesBoton(...)}>). */
export function clasesBoton(variante: VarianteBoton = 'primario', tamano: 'normal' | 'chico' = 'normal'): string {
  const medidas = tamano === 'chico' ? 'px-3 py-1.5 text-xs' : 'px-4 py-2 text-sm';
  return `inline-flex items-center justify-center rounded-md font-semibold shadow-sm focus-visible:outline-2 focus-visible:outline-offset-2 disabled:cursor-not-allowed disabled:opacity-60 ${medidas} ${estilosBoton[variante]}`;
}
...
export function Boton({
  ...
  className = '',
  ...props
}: ComponentProps<'button'> & { variante?: VarianteBoton; tamano?: 'normal' | 'chico' }) {
  return <button type="button" className={`${clasesBoton(variante, tamano)} ${className}`} {...props} />;
}
